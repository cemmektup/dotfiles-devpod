...
vim.api.nvim_set_keymap('n', '<Leader>ee', ':OverseerToggle<CR>',
...
vim.keymap.set('n', '<Leader>em',
  function()
    local phpstan = [[make\ toolbox.phpstan-raw\ \\\|\ grep\ \"src/\\\\\|tests/\"]]
    vim.cmd("set makeprg=" .. phpstan)
    vim.cmd("make")
  end,
  { silent = true, noremap = true, desc = "[E]xecute [M]ake phpstan" })
